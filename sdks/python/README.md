...
`--study-python` is a convenience launcher (`python3 …`); `--study study.py` and
`--study-uv` run via `uv run …` (PEP 723 inline metadata), and
`--study-cmd "python3 study.py"` still works for an arbitrary command line.
