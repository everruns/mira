...
```bash
mira list --example my_evals                 # what the study advertises
mira run --example my_evals                  # run the whole matrix
mira run --example my_evals greet            # selective (substring), like cargo test
mira run --example my_evals --tag smoke
mira run --example my_evals --format html --out report.html   # self-contained viewer
mira run --example my_evals                                   # saves a run folder by default
mira run --example my_evals --resume <run_id>                 # resume; run only the missing cases
