...
```bash
mira run --script examples/swe_bench.rs --group-by difficulty   # one resolve-rate row per difficulty
mira run --script examples/swe_bench.rs --group-by agent         # …or per model-level config key
```
